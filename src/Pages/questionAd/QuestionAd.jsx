...
const QuestionAd = () => {
  ...
  const axiosJwt = createAxios(admin, dispatch, token);

  const totalPages = questionAd.questionAd
    ? questionAd.questionAd.totalPages
    : 1;
  ...
  useEffect(() => {
    if (admin && token) {
      dispatch(getQuesAd(token, page, axiosJwt));
    }
  }, [page, questionId, questionUpdate]);
  const handlePrev = () => {
    if (page <= 1) {
      setPage(1);
      return;
  ...
  return (
    <div>
      ...
      <div className="question-container">
        ...
        {questionAd && questionAd.questionAd
          ? questionAd.questionAd.results.map((item, index) => {
              return (
                <QuestionAdItem
                  data={item}
                  key={item.id}
